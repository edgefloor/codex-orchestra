...
const paths = (environment: Environment) => ({
  executable: path.join(environment.resourcesPath, "orchestra", "orchestra-product"),
  manifest: path.join(environment.resourcesPath, "orchestra", "release-manifest.json"),
  policyRoot: path.join(environment.resourcesPath, "orchestra"),
  state: path.join(environment.stateDir, "orchestra-product-install-state.json"),
  codexHome: process.env.CODEX_HOME?.trim() || path.join(environment.homeDirectory, ".codex"),
  appBundle: path.resolve(environment.resourcesPath, "..", ".."),
});

const run = (environment: Environment, operation: string, args: ReadonlyArray<string>) =>
  Effect.try({
    try: () => {
      const result = spawnSync(paths(environment).executable, args, {
        encoding: "utf8",
