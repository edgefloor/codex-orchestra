...
- Source revision: unavailable; the supplied candidate directory has no Git metadata
- Tree status: unavailable for the same reason
...
| Unit tests | `uv run --locked python -m unittest discover -s tests -v` | pass | 15 tests passed in 0.041s |
