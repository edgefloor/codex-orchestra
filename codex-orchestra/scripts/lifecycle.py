...
def desired_project(target: Path) -> list[DesiredFile]:
    source = PLUGIN / "config/project"
    files = [DesiredFile(source / "config.toml", target / ".codex/config.toml")]
    files.extend(
...
def desired_global(home: Path, *, default: bool) -> list[DesiredFile]:
    source = PLUGIN / "config/global"
    config_name = "config.toml" if default else "orchestra.config.toml"
    files = [DesiredFile(source / "orchestra.config.toml", home / config_name)]
    files.extend(
...
def doctor() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            shutil.copy2(PLUGIN / "config/project/config.toml", Path(tmp) / "config.toml")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            shutil.copy2(PLUGIN / "config/global/orchestra.config.toml", Path(tmp) / "orchestra.config.toml")
            env = dict(os.environ, CODEX_HOME=tmp)
            result = subprocess.run(
