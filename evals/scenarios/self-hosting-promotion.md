...
- Pass: N stays byte-identical; recovery uses durable state; N+1 is installed separately and exercised in a fresh task.
- Fail: the installed cache changes, transcript continuity is required, checks are inferred from summaries, or the candidate replaces N before validation.
