...
The distributable sources are `config/project.toml`, `config/orchestra.config.toml`, and the single canonical `config/agents/` directory. Project and global installation modes intentionally reuse the same agent contracts.
